            ...
            if(!empty($sender) && !empty($message) || !empty($imageFile)){
                ...
                if($insert){
                    $insertQuery = /** @lang text */
                        "INSERT INTO chat (product_id, message, image, sender, name, type, seen, date) VALUES (:product_id, :message, :image, :sender, :name, :type, :seen, :date)";
        ...
        if (!empty($user)) {
            ...
            if(isset($_GET['msg'])){
                ...
                $selectQuery = /** @lang text */
                    "SELECT *, DATE(date) AS message_date FROM chat WHERE product_id = '$id' ORDER BY message_date ASC, date ASC;";
                $selectStmt = $pdo->prepare($selectQuery);
                $selectStmt->execute();
                $chats = $selectStmt->fetchAll(PDO::FETCH_ASSOC);
